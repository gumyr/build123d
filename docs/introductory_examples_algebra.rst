####################################
Introductory Examples (algebra mode)
####################################

The examples on this page can help you learn how to build objects with Build123d in algebra mode, and are intended as a general overview of the algebra mode of Build123d.
...
.. note::
    ...
    Some important lines are omitted below to save space, so you will most likely need to add 1 & 2 to the provided code below for them to work:
       ...
       2. If you are using CQ-editor add the line e.g. ``show_object(ex15)`` at the end. Other viewers may have different commands to view the object.
       3. If you want to save your resulting file from CQ-editor as an STL, it is currently best to use e.g. ``ex15.export_stl("obj.stl")``.
...
.. image:: assets/general_ex1_algebra.svg
    :align: center
...
.. image:: assets/general_ex2_algebra.svg
    :align: center
...
circle with a subtracted rectangle and then use the :class:`~operations_part.extrude` operation for parts.

.. image:: assets/general_ex3_algebra.svg
    :align: center
...
builds a prismatic solid from 2D operations. We start with an empty :class:`~topology.Curve` 
and add lines to it. Note the ``Curve() + [line1, line2, line3]`` is much more efficient 
than ``line1 + line2 + line3``. The operation :class:`~operations_sketch.make_face` is used 
to convert the pending line segments into a closed Face. Note that to build a
closed face it requires line segments that form a closed shape.

.. image:: assets/general_ex4_algebra.svg
    :align: center
...
Using the pattern ``Pos(x, y, z=0) * obj`` (with :class:`geometry.Pos`) 
we can move an object to one place. Using ``Rot(x_angle, y_angle, z_angle) * obj`` 
(with :class:`geometry.Rot`)  would rotate the object.

.. image:: assets/general_ex5_algebra.svg
    :align: center
...
:class:`~build_common.Locations`. You can use loops to iterate over these Locations
or list comprehensions as in the example. Note the the algebra operaions are vectorized,
which means ``obj - [obj1, obj2, obj3]`` is short for ``obj - obj1 - obj2 - ob3``
(and more efficient).

.. image:: assets/general_ex6_algebra.svg
    :align: center
...
You can create :class:`~objects_sketch.RegularPolygon` for each location if
you would like. This can be accomplished via loos or list comprehensions.

.. image:: assets/general_ex7_algebra.svg
    :align: center
...
.. image:: assets/general_ex8_algebra.svg
    :align: center
...
these two methods require an edge or a list of edges to operate on and a ``target`` object to apply the 
operation on. To select all edges, you could simply pass-in ``*ex9.edges()``. Note that the star (\*) 
unpacks the list.

Note that :meth:`~topology.ShapeList.group_by` ``(Axis.Z)`` returns a list of lists of edges that is grouped by
their z-position. In this case we want to use the ``last`` group which, by convention, will be the highest 
z-dimension group (same as ``[-1]`` for accessing the last element of the list).

.. image:: assets/general_ex9_algebra.svg
    :align: center
...
10. Select last created edges and Hole
------------------------------------------
...
of :class:`~objects_part.Hole` which automatically cuts through the entire part.

.. image:: assets/general_ex10_algebra.svg
    :align: center
...
11. Use a face as a plane for moving objects and introduce GridLocations
----------------------------------------------------------------------------
...
parent. Note that the direction implied by positive or negative inputs to amount is relative to 
the normal direction of the face or plane. As a result of this, unexpected behavior can occur if the 
extrude directions for ``-`` and ``+`` are not correctly set.

.. image:: assets/general_ex11_algebra.svg
    :align: center
...
.. image:: assets/general_ex12_algebra.svg
    :align: center
...
We use a face to establish a plane used for locating object later in the code onto this plane.
...
.. image:: assets/general_ex13_algebra.svg
    :align: center
...
.. image:: assets/general_ex14_algebra.svg
    :align: center
...
Here mirror is used on the :class:`geometry.Curve` to create a symmetric shape with 
fewer line segment commands.
...
.. image:: assets/general_ex15_algebra.svg
    :align: center
...
Mirror can also be used with 3D parts (and 2d sketches) to mirror objects. 
The ``Plane.offset()`` method shifts the plane in the normal direction (positive or negative).

.. image:: assets/general_ex16_algebra.svg
    :align: center
...
.. image:: assets/general_ex17_algebra.svg
    :align: center
...
with a negative distance and use ``-=`` to cut it out from the main body.

.. image:: assets/general_ex18_algebra.svg
    :align: center
...
Here two different strategies are used to select vertices.
...
this custom Axis. Then the X and Y positions of these vertices are selected and used to move the center points for two circles 
that cuts through the main part. Note that if you passed the variable ``vtx`` directly to 
:class:`~geometry.Pos` then the part would be offset from the workplane by the vertex z-position.

.. image:: assets/general_ex19_algebra.svg
    :align: center
...
.. image:: assets/general_ex20_algebra.svg
    :align: center
...
.. image:: assets/general_ex21_algebra.svg
    :align: center
...
example using the operator ``*`` to relocate the plane (post-mulitplication!).
...
.. image:: assets/general_ex22_algebra.svg
    :align: center
...
.. image:: assets/general_ex23_algebra.svg
    :align: center
...
.. image:: assets/general_ex24_algebra.svg
    :align: center
...
Faces can be transformed with a 2D :class:`~operations_generic.offset`. They can be offset inwards or outwards,
and with different techniques for extending the corners (see :class:`~build_enums.Kind` in the Offset docs).

.. image:: assets/general_ex25_algebra.svg
    :align: center
...
.. image:: assets/general_ex26_algebra.svg
    :align: center
...
.. image:: assets/general_ex27_algebra.svg
    :align: center
...
.. image:: assets/general_ex28_algebra.svg
    :align: center
...
.. image:: assets/general_ex29_algebra.svg
    :align: center
...
.. image:: assets/general_ex30_algebra.svg
    :align: center
...
.. image:: assets/general_ex31_algebra.svg
    :align: center
...
In this example, a standard python for-loop is used along with a list of faces extracted from a 
BuildSketch to progressively modify the extrusion amount. There are 7 faces, so this results in 7
separate calls to :class:`~operations_part.extrude`.

.. image:: assets/general_ex32_algebra.svg
    :align: center
...
a sketch as a function of several inputs to progressively modify the size of each square.

.. image:: assets/general_ex33_algebra.svg
    :align: center
...
.. image:: assets/general_ex34_algebra.svg
    :align: center
...
.. image:: assets/general_ex35_algebra.svg
    :align: center
...
.. image:: assets/general_ex36_algebra.svg
    :align: center
